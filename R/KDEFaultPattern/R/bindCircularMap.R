...
bindCircularMap = function(rectangularMap, dieWidth, dieHeight, waferRay, outValue = NA) {
  
  for (i in 1:nrow(rectangularMap)){
    for (j in 1:ncol(rectangularMap)){
      if ((-dieWidth/2 + i*dieWidth - waferRay)^2 + (-dieHeight/2 + j*dieHeight - waferRay)^2 >= waferRay^2)
